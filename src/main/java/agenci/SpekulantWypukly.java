...
public class SpekulantWypukly extends Spekulant {
    ...
    public SpekulantWypukly(int id, Zasoby zasoby) {
        super(id, zasoby);
    }

    @Override
    public KarieraJson karieraJson() {
        return new KarieraJson("wypukly", -1);
    }
    ...
    public void wystawOferty(Historia h) {
        ...
        for (Produkt p : produkty) {
            ...
            if (h.czyCenyWypukle(p)) {
                for (int jakosc = 1; jakosc <= h.dajDzien(); jakosc++) {
                    ofertyKupna.add(new OfertaSpekulanta(p, 100, jakosc, sredniaCena * 0.9, this));
                }
            }
            if (h.czyCenyWklesle(p)) {
                for (int jakosc = 1; jakosc <= h.dajDzien(); jakosc++) {
                    Dobra dobra = zasoby.dajProdukt(p).size() >= jakosc ? zasoby.dajProdukt(p).get(jakosc - 1) : null;
                    if (dobra == null)
                        continue;
                    ofertySprzedazy.add(new OfertaSpekulanta(p, dobra.ile(), jakosc, sredniaCena * 1.1, this));
                }
            }
        }
